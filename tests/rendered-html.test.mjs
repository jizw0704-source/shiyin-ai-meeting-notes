...
test("supports local transcription and keeps cloud keys behind the realtime proxy", async () => {
  ...
  assert.match(page, /移入最近删除/);
  assert.match(page, /恢复所选/);
  assert.match(page, /function returnFromMeetingDetail/);
  assert.match(page, /返回本机工作区/);
  assert.match(page, /返回会议首页/);
  assert.match(page, /function meetingTitleSuggestions/);
  assert.match(page, /智能命名建议/);
  assert.match(page, /onContextMenu/);
  assert.match(page, /打开并导出报告/);
  assert.match(page, /meeting-brief-conclusion/);
