...
- 超过 1 秒的语音间隔显示为停顿。
- 本地 CAM++ 声纹嵌入与增量聚类，发言人自动编号；人工命名后会写入本机声纹库，后续会议可保守地自动匹配姓名。
- 新会议可选择 6、12 或 20 人上限，实时识别、会后校正和历史重新转写使用同一设置。
- 左侧历史会议支持按名称或日期搜索，点击即可快速查看；会议名称可在列表中直接重命名。
- 应用启动后停留在独立的会议准备页，不自动打开历史记录；中央“开始会议”按钮可直接进入本地听记。
- 点击发言人姓名手动重命名；会后重新聚类时尽量保留人工姓名，自动匹配结果会明确标记且仍可修改。
- 逐字稿提供“整理稿/原始记录”和“正序/倒序”切换，可保守过滤“嗯、啊、呃”等口语词；显示顺序不会改写原始记录。
...
- “本机存储”支持创建带完整性校验的会议备份，并将缺失会议和本机发言人声纹库安全合并恢复；MiniMax 密钥不会进入备份。
...
完整备份可在左侧“本机存储”中创建。每个备份都是独立文件夹，包含会议录音、逐字稿、总结、发言人、本机声纹库与转写版本，并通过 SHA-256 校验文件完整性；从备份恢复时，已有会议会被跳过，不会覆盖当前数据。历史会议右上角的“转写版本”可重新运行本地模型，旧版本会自动保留。
...
API Key 只由本地 Node 后台读取，不会发给浏览器。语音转写、声纹识别、跨会议姓名匹配、原始录音和逐字稿都留在本机；只有整理后的文本会发送给 MiniMax 生成总结。人工替换和口语过滤使用独立整理层，原始识别文本始终保留。若不配置 MiniMax，录音和逐字稿仍会正常保存，之后可补充密钥再生成总结。

## 当前边界

说话人识别按完整 ASR 句段分配，不能可靠拆分两人同时说话的重叠语音。短于约 1.2 秒的片段会继承相邻发言人；安静环境、让每人至少连续说几句话，会明显提高区分效果。跨会议姓名匹配是便于整理的辅助能力，不用于身份认证；相似度不足或多个候选过于接近时会放弃自动命名。当前本地 Paraformer 模型不提供逐词时间戳，句段时间由实时音频位置估算。
