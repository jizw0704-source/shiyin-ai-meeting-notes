...
export class MeetingStorage {
  ...
  saveSummary(meetingId, summary) {
    ...
    this.syncMeetingMemories(meetingId, deriveMeetingMemoryCandidates(summary, meeting));
  }

  backfillMeetingMemories() {
    const rows = this.db.prepare(`
      SELECT id, summary_json FROM meetings
      WHERE summary_json IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM meeting_memories WHERE meeting_id = meetings.id)
    `).all();
    for (const row of rows) {
      try {
        const meeting = this.getMeeting(row.id);
        const summary = JSON.parse(row.summary_json);
        this.syncMeetingMemories(row.id, deriveMeetingMemoryCandidates(summary, meeting));
      } catch {
        // Older or incomplete summaries stay untouched and can be regenerated by the user.
      }
    }
  }

  syncMeetingMemories(meetingId, candidates = []) {
    const meeting = this.getMeeting(meetingId);
    if (!meeting) throw new Error("会议不存在");
    const normalized = Array.isArray(candidates) ? candidates.slice(0, 24) : [];
    const activeKeys = new Set(normalized.map((item) => item.sourceKey));
    const now = new Date().toISOString();
    const existing = this.db.prepare("SELECT source_key, status FROM meeting_memories WHERE meeting_id = ?")
      .all(meetingId);
    const insert = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)
      ON CONFLICT(meeting_id, source_key) DO UPDATE SET
        kind = excluded.kind,
        content = CASE WHEN meeting_memories.status = 'pending' THEN excluded.content ELSE meeting_memories.content END,
        confidence = CASE WHEN meeting_memories.status = 'pending' THEN excluded.confidence ELSE meeting_memories.confidence END,
        evidence_seqs_json = CASE WHEN meeting_memories.status = 'pending' THEN excluded.evidence_seqs_json ELSE meeting_memories.evidence_seqs_json END,
        updated_at = CASE WHEN meeting_memories.status = 'pending' THEN excluded.updated_at ELSE meeting_memories.updated_at END
    `);
    this.db.exec("BEGIN");
    try {
      for (const item of normalized) {
        insert.run(
          ...
          item.content,
          item.confidence,
          JSON.stringify(item.evidenceSeqs || []),
          now,
          now,
        );
      }
      for (const row of existing) {
        if (row.status === "pending" && !activeKeys.has(row.source_key)) {
          this.db.prepare("DELETE FROM meeting_memories WHERE meeting_id = ? AND source_key = ? AND status = 'pending'")
            .run(meetingId, row.source_key);
        }
      }
      this.db.exec("COMMIT");
    } catch (error) {
    ...
    return this.listMemories({ meetingId });
  }

  hydrateMemory(row) {
    const seqs = JSON.parse(row.evidence_seqs_json || "[]");
    const evidenceRows = seqs.length
      ? this.db.prepare(`
        SELECT seq, COALESCE(NULLIF(edited_text, ''), text) AS text, start_ms
        FROM segments WHERE meeting_id = ? AND seq IN (${seqs.map(() => "?").join(",")})
        ORDER BY seq
      `).all(row.meeting_id, ...seqs)
      : [];
    return {
      id: row.id,
      meetingId: row.meeting_id,
  ...
  listMemories(options = {}) {
    ...
    if (options.meetingId) {
      where.push("meeting_memories.meeting_id = ?");
      values.push(options.meetingId);
    }
    if (["pending", "confirmed"].includes(options.status)) {
      where.push("meeting_memories.status = ?");
    ...
    return this.db.prepare(`
      ...
      LIMIT ?
    `).all(...values, limit).map((row) => this.hydrateMemory(row));
  }
  ...
  getMemoryStats() {
    ...
    for (const row of rows) {
      ...
      if (row.status === "confirmed") result.confirmed = count;
    }
    return result;
  }
  ...
  updateMemory(id, patch) {
    ...
    const normalized = normalizeMeetingMemoryPatch(patch);
    if (Object.hasOwn(normalized, "content")) normalized.status = "confirmed";
    const entries = Object.entries(normalized);
    if (entries.length) {
      const columns = { content: "content", status: "status" };
      this.db.prepare(`UPDATE meeting_memories SET ${entries.map(([key]) => `${columns[key]} = ?`).join(", ")}, updated_at = ? WHERE id = ?`)
  ...
  dismissMemory(id) {
    if (!this.getMemory(id)) return null;
  ...
  importMeetingSnapshot(snapshot, options = {}) {
    ...
    try {
      ...
      for (const memory of snapshot.memories || []) {
        const kind = ["person", "organization", "project", "decision", "need", "term"].includes(memory.kind)
          ? memory.kind
          : null;
        const status = ["pending", "confirmed", "dismissed"].includes(memory.status)
          ? memory.status
          : "pending";
        ...
        `).run(
          ...
          meetingId,
          String(memory.sourceKey || `${kind}:restored:${randomUUID()}`).slice(0, 120),
          kind,
          String(memory.content).trim().slice(0, 280),
          status,
